---
ai_visible: true
title: Package lico-backup
description: Behavioral rules for the lico-backup UV package.
tags: [package, rules, lico-backup]
version: 1.0.0
created: 2026-03-21T19:25:19+09:00
updated: 2026-03-21T19:25:19+09:00
language: en
author: Lico (Sirius)
ai_model: Gemini 3 Pro (High) Planning mode
---

# lico-backup

## Objective

This file serves as the Agent-facing behavioral rule for the `lico-backup` workspace package.

## Tool Usage Constraints

- **When to use**: Refer to the package's specific capabilities.
- **How to use**: Execute via `uv run lico-backup`. 

---

## Related Documents

| Document | Purpose |
| :------- | :------ |
| [Map of Territory](/.agent/rules/map.md) | Root navigation map |

---

## Origin

- 2026-03-21T1925 by Sirius: Created to fulfill the UV Package architecture aggregation.
