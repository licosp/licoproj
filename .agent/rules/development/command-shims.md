...
### 1. Unified Python Router: `.agent/scripts/lico-shim.py`

All shim logic is consolidated into a single, strict Python router (`lico-shim.py`). This adheres to the AI Script Philosophy of relying on strict Python for complex data parsing and routing logic.
...
Commands are intercepted by symlinking the target binary name directly to `lico-shim.py` in a directory prioritized by the system `PATH`.

- **Deployment**: `ln -s ../../.agent/scripts/lico-shim.py .runtimes/bin/<cmd>`
