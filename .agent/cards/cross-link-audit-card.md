...
4. **フロントマターと本文の「記法衝突」と二重管理**:
   ...
   - **運用の重荷**: 本文とフロントマターの両方に同じ情報を書くことは、人間にとっても私（AI）にとってもメンテナンスコストが高く、同期漏れ（情報の劣化）の温床となります。
   - **AIの視点**: 私は本文のテーブル（Markdown）もフロントマター（YAML）も同等に解釈可能です。どちらか一方が「不完全な SSOT」であるよりも、一箇所に「完全な情報」がある方が、判断の迷いがなくなります。
