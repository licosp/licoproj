...
- 文章の構造は、以下の順番で書くように行動規範に定められているでしょうか?
  1. `## Origin`
  2. `## Related Documents`
  3. `**Navigation**`
- それぞれに表記ゆれが存在する可能性があります。
- `Navigation` と `Related Documents` は統一すべきでしょうか?
- `Navigation` の表記も `## Navigation` に統一すべきでしょうか?
- 文脈化された定型作業です。（`rules-update-card.md`）
...
- 文脈化された非定型作業です。（`drafts-cleanup-card.md`）
